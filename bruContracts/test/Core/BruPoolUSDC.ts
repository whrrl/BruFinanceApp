...
describe("BruPool Tests", async () => {
    before(async function () {
		...
		await CONTRACTS.USDCTestToken.mint(SIGNERS.SignerB.address, CONSTANTS.MINT_AMOUNT);
	...
	it("Deposit 3000 USDC Tokens and Withdraw the matured bond", async() => {
		await CONTRACTS.USDCTestToken.approve(PROXIES.BruPoolProxy.address, CONSTANTS.MINT_AMOUNT);
		await PROXIES.BruRouterProxy.deposit(0, CONTRACTS.USDCTestToken.address, CONSTANTS.DEPOSIT_AMOUNT);
	...
	it("Redeem Interest Token", async() => {
    ...
    it("Interest Example", async () => {
        ...
        expect("2.876712328767123287").to.equal(ethers.utils.formatUnits(interest, 18));
