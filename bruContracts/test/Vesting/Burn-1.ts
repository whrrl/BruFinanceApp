...
import {deploy, SIGNERS, PROXIES,getProposalCounter,
    increaseProposalCounter, CONTRACTS, CONSTANTS} from "../shared/deploy-1";
...
describe("Burn Mechanism", async () => {
    ...
    it("should burn 5000 tokens from category", async () => {
        ...
        console.log("%c Line:47 🥝 burnTokenAmount", "color:#42b983", burnTokenAmount);
    ...
    it("issue new tokens according  to TVL ", async function () {
        ...
        await CONTRACTS.TestTokenContract.mint(PROXIES.BruPoolProxy.address, "125000000000000");
        ...
        let burnTokenAmount = ethers.utils.parseUnits("500000", 6);
        console.log("%c Line:82 🍓 burnTokenAmount", "color:#ffdd4d", burnTokenAmount);
        await PROXIES.TokenVestingBruProxy.burnTokensFromCategory(1, burnTokenAmount);
        await expect(
            PROXIES.TokenVestingBruProxy.burnTokensFromCategory(1, "1000000000000000000000000000")
        ).to.be.revertedWith("this category does not have enough tokens");

        await CONTRACTS.TestTokenContract.mint(PROXIES.BruPoolProxy.address, "30000000000000");
